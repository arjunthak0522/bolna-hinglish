...
new="const stream=await getMicStream();const track=stream.getAudioTracks()[0];if(track?.muted)await Promise.race([new Promise(resolve=>track.addEventListener('unmute',resolve,{once:true})),new Promise(resolve=>setTimeout(resolve,450))]);const mime=pickMime();recorder="
...
new="if(b.size<256){note('microphone',{phase:'empty_recording',bytes:b.size});releaseMicrophone();error={title:'Microphone wasn’t ready',detail:'Safari did not deliver audio that time. Tap the microphone again and Bolna will reacquire it.'};state='idle';return render()}"
